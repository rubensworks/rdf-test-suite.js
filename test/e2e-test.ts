...
describe('e2e tests on the test suite runner', () => {
  ...
  // describe('query', () => {
  //   it('should run correctly on [http://w3c.github.io/rdf-tests/sparql/sparql11/manifest-all.ttl]', async () => {
  ...
  //     expect(console.warn).not.toHaveBeenCalled();
  //     // expect(console.warn).toHaveBeenCalledWith(new Error("Could not parse the query result with content type text/tab-separated-values at http://w3c.github.io/rdf-tests/sparql/sparql11/csv-tsv-res/csvtsv02.tsv"))
  ...
  //     // runner.resultsToText(process.stdout, result, false);
  //     console.log(result)
...
//                   .map(([ key, value ]) => [ `?${key.value}`, value ]))), true
//         );
//       } else {
